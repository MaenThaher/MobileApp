...
import { patchInstructorAssignment } from "@/services/instructorService";
import type { Assignment, AssignmentStatus, CircuitTemplate } from "@/types";
...
async function onSubmit(
  ...
  await patchInstructorAssignment(
    courseId,
    assignmentId,
    instructorId,
    formData
...
export function EditAssignmentModal({
  ...
  // Pre-fill form when modal opens or assignment changes
  useEffect(() => {
    if (visible && initialValues) {
      ...
      setMaxPoints(String(initialValues.max_points || 100));
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      await onSubmit(courseId, initialValues.id, instructorId, formData);
      ...
      setErrors({
        root: error.message || "Failed to update assignment. Please try again.",
      });
    } finally {
